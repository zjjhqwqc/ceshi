...
import top.niunaijun.blackboxa.util.PackageVisibilityCompat
    ...
    fun previewInstallList() {
        try {
            synchronized(mInstalledList) {
                // 使用兼容方法获取已安装应用列表
                // 在 Android 15/16 上，标准 API 可能返回空列表
                // PackageVisibilityCompat 会自动尝试多种方法获取完整列表
                val installedApplications: List<ApplicationInfo> =
                    PackageVisibilityCompat.getInstalledApplicationsCompat(getPackageManager())
                Log.d(TAG, "previewInstallList: found ${installedApplications.size} apps")
